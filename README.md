...
- **Language** — `src/lib/i18n/`. UI chrome (nav, CTAs, labels) is translated for `en`/`fr`/`es`/`de`.
  Long-form editorial copy (hero statement, studio philosophy, case studies) intentionally stays in English
  across locales for now — see the comment in `LocaleProvider.tsx`.
